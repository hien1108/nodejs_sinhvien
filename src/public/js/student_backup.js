...
function openStudentModal(id = '', studentId = '', name = '', dob = '', gender = '', maKhoa = '', classId = '', email = '', phone = '', address = '') {
  ...
  document.getElementById('studentDob').value = dob ? dob.split('/').reverse().join('-') : '';
...
async function handleStudentFormSubmit(e) {
  ...
  try {
    ...
    if (response.ok) {
      showToast(result.message || `${isEdit ? 'Cập nhật' : 'Thêm'} sinh viên thành công!`, 'success');
      closeStudentModal();
      setTimeout(() => location.reload(), 1000);
    } else {
      showToast(result.message || `Lỗi khi ${isEdit ? 'cập nhật' : 'thêm'} sinh viên`, 'error');
    }
  } catch (error) {
...
if (document.readyState === 'loading') {
  ...
  initStudentManagement();
      ...
      <td>
        <button onclick="editStudent('${sv._id}', '${sv.maSV}', '${sv.tenSV}', '${sv.ngaySinh}', '${sv.gioiTinh}', '${sv.khoa}', '${sv.lop}', '${sv.email}', '${sv.sdt}', '${sv.diaChi}')">Sửa</button>
        <button onclick="deleteStudent('${sv._id}')">Xóa</button>
      </td>
    `;
